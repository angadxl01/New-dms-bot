...
# Keep secrets outside source code. Set these environment variables before starting the bot.
BOT_TOKEN = os.getenv("BOT_TOKEN", "")
MONGO_URL = os.getenv("MONGO_URL", "")

ADMINS = [8895089247] 
bot = Client("@Aoultetstidsssbot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN, in_memory=True)
...
# Pending join-request approval tasks: user_id -> task state
PENDING_REQUEST_TASKS = {}
# Persistent auto-accept workers: user_id -> asyncio.Task
AUTO_PENDING_TASKS = {}
...
async def get_user(user_id):
    ...
    if not user:
        user = {"_id": user_id, "username": "", "banned": False, "premium_expiry": 0, "sessions": [], "saved_channels": [], "custom_msg_type": "text", "custom_msg": "HELLO", "custom_caption": "", "total_dms": 0, "daily_dms": 0, "last_date": today, "messaged_users": {}, "pending_chats": [], "auto_pending_requests": []}
...
# ================= PENDING JOIN REQUEST ACCEPTOR =================
async def _get_pending_user_ids(userbot, chat_id):
    """Return currently pending join-request user IDs."""
    ids = []
...
async def auto_accept_pending_loop(user_id, entry):
    """Poll a configured chat and approve only requests that arrive after auto-accept is enabled."""
    user_id = str(user_id)
    session_string = entry.get("session")
    chat_id = entry.get("chat_id")
    chat_title = entry.get("chat_title", "Target")
    client_name = f"auto_preq_{user_id}_{int(time.time())}"
    userbot = None
    try:
        userbot = Client(
            client_name,
        ...
        seen = {int(x) for x in entry.get("seen_ids", []) if str(x).lstrip("-").isdigit()}
        # Rebuild the baseline on first start. This prevents auto-accept from clearing
        # requests that were already waiting before the feature was enabled.
        if not entry.get("baseline_ready"):
            current = await _get_pending_user_ids(userbot, chat_id)
            seen = set(current)
            entry["seen_ids"] = list(seen)[-2000:]
            entry["baseline_ready"] = True
            await users_col.update_one(
                {"_id": user_id},
                {"$set": {"auto_pending_requests": [entry]}},
                upsert=True,
            )
        ...
        while True:
            ...
            for target_user_id in new_ids:
                # If disabled from the UI, stop without processing more requests.
                saved = await users_col.find_one({"_id": user_id})
                configs = (saved or {}).get("auto_pending_requests", [])
                active = next((x for x in configs if str(x.get("chat_id")) == str(chat_id)), None)
                if not active or not active.get("enabled", False):
                    return

                try:
                    await userbot.approve_chat_join_request(chat_id, target_user_id)
                    seen.add(target_user_id)
                except FloodWait as fw:
                    wait_for = int(getattr(fw, "value", 0) or getattr(fw, "x", 0) or 1)
                    await asyncio.sleep(wait_for)
                except Exception:
                    # Keep failed IDs out of `seen` so the next polling cycle can retry.
                    pass

            # Drop IDs that are no longer pending. This keeps the stored set small and
            # also allows the same user to request again later.
            seen.intersection_update(current_set)
            entry["seen_ids"] = list(seen)[-2000:]
            await users_col.update_one(
                {"_id": user_id},
                {"$set": {"auto_pending_requests": [entry]}},
        ...
        print(f"[AUTO-PREQ] user={user_id} chat={chat_id}: {exc}")
    finally:
        try:
            if userbot:
                await userbot.stop()
            ...
            pass
        AUTO_PENDING_TASKS.pop(user_id, None)


async def enable_auto_pending_requests(user_id, chat_id, chat_title, session_string):
    """Enable persistent auto-accept for one target chat."""
    user_id = str(user_id)
    # Stop an older worker for this user before replacing its configuration.
    old_task = AUTO_PENDING_TASKS.pop(user_id, None)
    ...
    try:
        userbot = Client(
            f"auto_preq_setup_{user_id}_{int(time.time())}",
            api_id=API_ID,
            api_hash=API_HASH,
            session_string=session_string,
            in_memory=True,
        ...
        await userbot.start()
        baseline = await _get_pending_user_ids(userbot, chat_id)
    finally:
        try:
            if userbot:
                await userbot.stop()
    ...
    AUTO_PENDING_TASKS[user_id] = asyncio.create_task(auto_accept_pending_loop(user_id, entry))
    return len(baseline)


async def disable_auto_pending_requests(user_id):
    user_id = str(user_id)
    task = AUTO_PENDING_TASKS.pop(user_id, None)
    if task and not task.done():
        ...
        try:
            ...
            pass
    await users_col.update_one({"_id": user_id}, {"$set": {"auto_pending_requests": []}}, upsert=True)


async def restore_auto_pending_requests():
    """Restore auto-accept workers after a bot restart."""
    try:
        async for u in users_col.find({"auto_pending_requests.0": {"$exists": True}}):
            uid = str(u.get("_id"))
            entries = u.get("auto_pending_requests", [])
            if not entries:
                continue
            entry = entries[0]
            if not entry.get("enabled") or not entry.get("session") or not entry.get("chat_id"):
                continue
            if uid not in AUTO_PENDING_TASKS:
                AUTO_PENDING_TASKS[uid] = asyncio.create_task(auto_accept_pending_loop(uid, entry))
    except Exception as exc:
        print(f"[AUTO-PREQ] restore error: {exc}")


async def get_pending_request_count(userbot, chat_id):
    """Return the number of currently pending join requests."""
    count = 0
    try:
        async for _ in userbot.get_chat_join_requests(chat_id):
            count += 1
    except Exception:
        # Some Telegram/Pyrogram versions expose the count through raw API.
        try:
            peer = await userbot.resolve_peer(chat_id)
            result = await userbot.invoke(GetChatInviteImporters(
                peer=peer,
                requested=True,
                offset_date=0,
...
async def approve_pending_requests_task(user_id, chat_id, chat_title, session_string, message_id):
    """Approve pending join requests one-by-one with pause/stop controls."""
    task = PENDING_REQUEST_TASKS.setdefault(str(user_id), {})
    task.update({
        "status": "running",
        "chat_id": chat_id,
    ...
    try:
        userbot = Client(
            f"preq_{user_id}_{int(time.time())}",
            api_id=API_ID,
            api_hash=API_HASH,
            session_string=session_string,
            in_memory=True,
        ...
        # Re-read requests so the task only acts on requests that are still pending.
        requests = []
        async for req in userbot.get_chat_join_requests(chat_id):
            if getattr(req, "user", None) and getattr(req.user, "id", None):
                requests.append(int(req.user.id))

        task["total"] = len(requests)
        if not requests:
            await api_edit(
                user_id,
                message_id,
                f"{E_CHK} <b>No pending join requests.</b>\n\n<b>Target:</b> {html.escape(str(chat_title))}",
                {"inline_keyboard": [[ibtn("Back", "pending_requests", style="primary", icon=BTN_EMOJIS["home"])]]}
            )
            return

        for target_user_id in requests:
            # Pause/stop is checked before every approval.
            ...
            try:
                # Pyrogram's official method requires admin + can_invite_users permission.
                await userbot.approve_chat_join_request(chat_id, target_user_id)
                task["approved"] += 1
            except FloodWait as fw:
                wait_for = int(getattr(fw, "value", 0) or getattr(fw, "x", 0) or 1)
                ...
                await api_edit(
                    user_id,
                    message_id,
                    f"{E_WAIT} <b>Telegram rate limit reached.</b>\nWaiting <b>{wait_for}s</b> before continuing...",
                    {"inline_keyboard": [[ibtn("Resume", "preq_resume", style="success", icon=BTN_EMOJIS["play"] if "play" in BTN_EMOJIS else BTN_EMOJIS["start"]), ibtn("Stop", "preq_stop", style="danger", icon=BTN_EMOJIS["cross"])]]}
                )
                await asyncio.sleep(wait_for)
                ...
                # A request can disappear while processing; count it and continue.
                task["failed"] += 1

            done = task["approved"] + task["failed"]
            if done % 10 == 0 or done == task["total"]:
                await api_edit(
                    user_id,
                    message_id,
                    f"{E_SYNC} <b>Accepting Pending Requests...</b>\n\n"
                    f"👥 <b>Target:</b> {html.escape(str(chat_title))}\n"
                    f"{E_CHK} <b>Approved:</b> {task['approved']}\n"
                    f"{E_WRN} <b>Failed/Expired:</b> {task['failed']}\n"
                    f"📊 <b>Progress:</b> {done}/{task['total']}",
                    {"inline_keyboard": [[ibtn("Pause", "preq_pause", style="primary", icon=BTN_EMOJIS["start"]), ibtn("Stop", "preq_stop", style="danger", icon=BTN_EMOJIS["cross"])]]}
                )
            await asyncio.sleep(0.08)

        if task.get("status") == "stopped":
            final_title = f"{E_STOP} <b>Pending Request Accept Stopped</b>"
        else:
            task["status"] = "completed"
            final_title = f"{E_CHK} <b>Pending Request Accept Completed</b>"
        ...
        await api_edit(
            ...
            f"{final_title}\n\n"
            f"👥 <b>Target:</b> {html.escape(str(chat_title))}\n"
            f"{E_CHK} <b>Approved:</b> {task.get('approved', 0)}\n"
            f"{E_WRN} <b>Failed/Expired:</b> {task.get('failed', 0)}\n"
            f"📊 <b>Total Found:</b> {task.get('total', 0)}",
            {"inline_keyboard": [[ibtn("Pending Requests", "pending_requests", style="primary", icon=BTN_EMOJIS["user"])], [ibtn("Back Home", "back_home", style="primary", icon=BTN_EMOJIS["home"])]]}
        )
    except FloodWait as fw:
        wait_for = int(getattr(fw, "value", 0) or getattr(fw, "x", 0) or 1)
        task["status"] = "stopped"
        await api_edit(user_id, message_id, f"{E_WRN} <b>Telegram rate limit:</b> wait {wait_for}s and try again.")
    except Exception as exc:
        task["status"] = "stopped"
        await api_edit(
            user_id,
            message_id,
            f"{E_WRN} <b>Pending Request Accept Error</b>\n\n<code>{html.escape(str(exc))}</code>\n\n"
            f"Make sure the selected account is an admin and has <b>Add Users / Invite Users</b> permission.",
            {"inline_keyboard": [[ibtn("Pending Requests", "pending_requests", style="primary", icon=BTN_EMOJIS["user"])], [ibtn("Back Home", "back_home", style="primary", icon=BTN_EMOJIS["home"])]]}
        )
    finally:
        try:
...
async def pending_request_preview(client, user_id, raw_link, sess_idx, message_id):
    """Resolve a target chat and show its current pending request count."""
    userbot = None
    try:
        u = await get_user(user_id)
        sessions = u.get("sessions", [])
        if not sessions:
            return await api_edit(user_id, message_id, f"{E_WRN} <b>No session found.</b> Add a session first.", {"inline_keyboard": [[ibtn("Add Session", "add_session", style="success", icon=BTN_EMOJIS["add"])], [ibtn("Back", "back_home", style="danger", icon=BTN_EMOJIS["cross"])]]})
        idx = int(sess_idx)
        if idx < 0 or idx >= len(sessions):
            return await api_edit(user_id, message_id, f"{E_WRN} Invalid session selection.", {"inline_keyboard": [[ibtn("Back", "pending_requests", style="primary", icon=BTN_EMOJIS["home"])]]})

        session_string = sessions[idx]
        userbot = Client(f"preq_check_{user_id}_{int(time.time())}", api_id=API_ID, api_hash=API_HASH, session_string=session_string, in_memory=True)
        await userbot.start()
        chat = await get_chat_safely(userbot, raw_link, raw_link)
        count = await get_pending_request_count(userbot, chat.id)
        title = getattr(chat, "title", None) or getattr(chat, "username", None) or str(chat.id)

        USER_STATES[user_id] = {"state": None, "pending_chat_id": chat.id, "pending_chat_title": title, "pending_sess_idx": idx, "pending_session": session_string}
        PENDING_REQUEST_TASKS.pop(user_id, None)

        text = (
            f"{E_WAIT} <b>Pending Join Requests</b>\n\n"
            f"👥 <b>Target:</b> {html.escape(str(title))}\n"
            f"🆔 <b>Chat ID:</b> <code>{chat.id}</code>\n"
            f"📥 <b>Pending Requests:</b> <b>{count}</b>\n\n"
            f"<i>The selected account must be an admin with Add Users / Invite Users permission.</i>"
        )
        auto_cfg = (await get_user(user_id)).get("auto_pending_requests", [])
        auto_on = any(str(x.get("chat_id")) == str(chat.id) and x.get("enabled") for x in auto_cfg)
        auto_label = "Auto Accept: ON" if auto_on else "Auto Accept: OFF"
        auto_action = "preq_auto_off" if auto_on else "preq_auto_on"
        auto_style = "danger" if auto_on else "success"
        if count:
            btn = {"inline_keyboard": [[ibtn(f"Accept All ({count})", "preq_accept", style="success", icon=BTN_EMOJIS["tick"])], [ibtn(auto_label, auto_action, style=auto_style, icon=BTN_EMOJIS["tick"])], [ibtn("Refresh Count", "preq_refresh", style="primary", icon=BTN_EMOJIS["globe"])], [ibtn("Back", "pending_requests", style="danger", icon=BTN_EMOJIS["cross"])]]}
        else:
            btn = {"inline_keyboard": [[ibtn(auto_label, auto_action, style=auto_style, icon=BTN_EMOJIS["tick"])], [ibtn("Refresh Count", "preq_refresh", style="primary", icon=BTN_EMOJIS["globe"])], [ibtn("Back", "pending_requests", style="danger", icon=BTN_EMOJIS["cross"])]]}
        await api_edit(user_id, message_id, text, btn)
    except Exception as exc:
        await api_edit(user_id, message_id, f"{E_WRN} <b>Could not read pending requests.</b>\n\n<code>{html.escape(str(exc))}</code>", {"inline_keyboard": [[ibtn("Back", "pending_requests", style="primary", icon=BTN_EMOJIS["home"])]]})
    finally:
        try:
            if userbot:
                await userbot.stop()
...
Developed by - @WG_PRINCEE"""
    ...
    btns = [
        ...
        [ibtn("Scrape Group", "scrape_group", style="primary", icon=BTN_EMOJIS["user"]), ibtn("Pending Requests", "pending_requests", style="success", icon=BTN_EMOJIS["user"])],
        [ibtn("Invite & Earn", "invite_earn", style="primary", icon=BTN_EMOJIS["money"]), ibtn("VIP Premium", "buy_premium", style="danger", icon=BTN_EMOJIS["diamond"])],
        [ibtn("My Account", "my_account", style="primary", icon=BTN_EMOJIS["user"]), ibtn("Add Session", "add_session", style="success", icon=BTN_EMOJIS["add"])],
        [ibtn("Remove Session", "remove_session", style="danger", icon=BTN_EMOJIS["cross"]), ibtn("Tutorial & Terms", "how_to_use", style="primary", icon=BTN_EMOJIS["setting"])],
...
async def cb_handler(client, query: CallbackQuery):
    ...
    if data == "coming_soon": pass
        ...
        sessions = u.get("sessions", [])
        if not sessions:
            return await safe_edit(query, f"{E_WRN} <b>Session Not Found!</b>\n\nAdd a Telegram session first. The session account must be an admin in the target chat with <b>Add Users / Invite Users</b> permission.", {"inline_keyboard": [[ibtn("ADD SESSION", "add_session", style="success", icon=BTN_EMOJIS["add"])], [ibtn("Back", "back_home", style="danger", icon=BTN_EMOJIS["cross"])]]})
        text = f"{E_WAIT} <b>Pending Request Acceptor</b>\n\nSelect the Telegram account/session that is an admin in the target chat:"
        btns = [[ibtn(f"Account {i+1}", f"preq_sess_{i}", style="primary", icon=BTN_EMOJIS["user"])] for i in range(len(sessions))]
        btns.append([ibtn("Back", "back_home", style="danger", icon=BTN_EMOJIS["cross"])])
        await safe_edit(query, text, {"inline_keyboard": btns})

    elif data.startswith("preq_sess_"):
        idx = int(data.split("_")[-1])
        u = await get_user(user_id)
        if idx < 0 or idx >= len(u.get("sessions", [])):
            return await query.answer("Invalid session.", show_alert=True)
        USER_STATES[user_id] = {"state": "WAITING_PENDING_LINK", "pending_sess_idx": idx}
        await safe_edit(query, f"{E_TRI} <b>Send Target Chat</b>\n\nSend the group/channel <b>username or link</b> where join requests are pending.\n\nExample: <code>@mygroup</code> or <code>https://t.me/mygroup</code>", {"inline_keyboard": [[ibtn("Cancel", "pending_requests", style="danger", icon=BTN_EMOJIS["cross"])]]})

    elif data == "preq_auto_on":
        state = USER_STATES.get(user_id, {})
        if not state.get("pending_chat_id") or not state.get("pending_session"):
            return await query.answer("Request setup expired. Open Pending Requests again.", show_alert=True)
        try:
            baseline = await enable_auto_pending_requests(
                user_id,
                state["pending_chat_id"],
                state.get("pending_chat_title", "Target"),
                state["pending_session"],
            )
            await query.answer("Auto Accept enabled.", show_alert=True)
            await safe_edit(
                query,
                f"{E_CHK} <b>Auto Accept Pending Requests: ON</b>\n\n"
                f"👥 <b>Target:</b> {html.escape(str(state.get('pending_chat_title', 'Target')))}\n"
                f"🛡 <b>Mode:</b> New requests only\n"
                f"📌 <b>Existing requests kept pending:</b> {baseline}\n\n"
                f"New join requests will be checked automatically every few seconds.\n"
                f"The selected account must remain an admin with <b>Add Users / Invite Users</b> permission.",
                {"inline_keyboard": [[ibtn("Turn OFF", "preq_auto_off", style="danger", icon=BTN_EMOJIS["cross"])], [ibtn("Refresh", "preq_refresh", style="primary", icon=BTN_EMOJIS["globe"])], [ibtn("Back", "pending_requests", style="primary", icon=BTN_EMOJIS["home"])]]}
            )
        except Exception as exc:
            return await query.answer(f"Could not enable: {str(exc)[:180]}", show_alert=True)

    elif data == "preq_auto_off":
        state = USER_STATES.get(user_id, {})
        try:
            await disable_auto_pending_requests(user_id)
            await query.answer("Auto Accept disabled.", show_alert=True)
            if state.get("pending_chat_id"):
                msg = await api_send(user_id, f"{E_SYNC} <b>Refreshing pending request settings...</b>")
                await pending_request_preview(client, user_id, str(state["pending_chat_id"]), state.get("pending_sess_idx", 0), msg.id)
                try:
                    await query.message.delete()
                except Exception:
                    pass
            ...
            return await query.answer(f"Could not disable: {str(exc)[:180]}", show_alert=True)

    elif data == "preq_accept":
        state = USER_STATES.get(user_id, {})
        if not state.get("pending_chat_id") or not state.get("pending_session"):
            return await query.answer("Request setup expired. Open Pending Requests again.", show_alert=True)
        if user_id in PENDING_REQUEST_TASKS and PENDING_REQUEST_TASKS[user_id].get("status") in {"running", "paused"}:
            return await query.answer("An accept task is already running.", show_alert=True)
        PENDING_REQUEST_TASKS[user_id] = {"status": "running", "chat_id": state["pending_chat_id"], "chat_title": state.get("pending_chat_title", "Target"), "approved": 0, "failed": 0, "total": 0}
        await safe_edit(query, f"{E_SYNC} <b>Preparing pending requests...</b>\nPlease wait.", {"inline_keyboard": [[ibtn("Pause", "preq_pause", style="primary", icon=BTN_EMOJIS["start"]), ibtn("Stop", "preq_stop", style="danger", icon=BTN_EMOJIS["cross"])]]})
        # Use the same message so progress updates remain in one place.
        asyncio.create_task(approve_pending_requests_task(user_id, state["pending_chat_id"], state.get("pending_chat_title", "Target"), state["pending_session"], query.message.id))

    elif data == "preq_pause":
        task = PENDING_REQUEST_TASKS.get(user_id)
        if not task or task.get("status") not in {"running", "paused"}:
            return await query.answer("No active accept task.", show_alert=True)
        task["status"] = "paused"
        await query.answer("Pending request accepting paused.")

    elif data == "preq_resume":
        task = PENDING_REQUEST_TASKS.get(user_id)
        if not task:
            return await query.answer("No active accept task.", show_alert=True)
        task["status"] = "running"
        await query.answer("Pending request accepting resumed.")
        ...
        if not task:
            return await query.answer("No active accept task.", show_alert=True)
        task["status"] = "stopped"
        await query.answer("Stopping after the current request...")

    elif data == "preq_refresh":
        state = USER_STATES.get(user_id, {})
        if not state.get("pending_chat_id") or not state.get("pending_session"):
            return await query.answer("Request setup expired. Open Pending Requests again.", show_alert=True)
        msg = await api_send(user_id, f"{E_SYNC} <b>Refreshing pending request count...</b>")
        await pending_request_preview(client, user_id, str(state["pending_chat_id"]), state.get("pending_sess_idx", 0), msg.id)
...
async def handle_states(client, message):
    ...
    try:
        ...
        if state == "WAITING_PENDING_LINK":
            raw_link = message.text.strip() if message.text else ""
            if not raw_link:
                return await api_send(user_id, f"{E_WRN} Send a valid group/channel username or link.")
            idx = USER_STATES[user_id].get("pending_sess_idx", 0)
            msg = await api_send(user_id, f"{E_SYNC} <b>Checking pending join requests...</b>")
            USER_STATES[user_id]["state"] = None
            await pending_request_preview(client, user_id, raw_link, idx, msg.id)

        elif state == "WAITING_SCRAPE_LINK":
...
async def start_bot():
    ...
    asyncio.create_task(restore_auto_pending_requests())
    await idle()
    print("Shutting down...")
    await bot.stop()
